...
function(n, q, k)
    local A5, dirProd, z, transvecDiag, T, subgroup;
    ...
    subgroup := Group(Concatenation([A5], GeneratorsOfGroup(dirProd), [T]));
    SetSize(subgroup, q ^ (k * (n-k)) * Size(SL(k, q)) * Size(SL(n-k, q)) *
    (q-1));
    return subgroup;
end);
